...
- **`/db/health` が `error` になる**: MySQL の初期化が終わるまで数秒かかるため、起動直後は `error` になる場合があります。数秒待ってから再度確認してください。
- **タイムゾーン関連の Warning**: MySQL 起動時に `/usr/share/zoneinfo` の Warning が出ることがありますが、開発用途では無視して問題ありません。
...
- フロントエンドは 5 秒ごとに `/health` と `/db/health` を再確認します。
